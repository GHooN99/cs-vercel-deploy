...
axiosClient.interceptors.response.use(
  ...
  (error) => {
    if (axios.isAxiosError(error) && error.response) {
      const response = error.response as AxiosResponse<CommonHttpResponse<unknown>>;
      ...
      if (isAuthenticationErrorCode(errorCodeOfFisrtError)) {
        return Promise.reject(new UnauthorizedException('UnauthorizedException'));
      }
      ...
      // 각 도메인의 에러
      if (errors) {
        return Promise.reject(new SpecificHttpException('SpecificHttpException', errors));
      }
    }
...
class CommonHttpAxiosClientImpl implements CommonHttpClient {
  ...
  async post<T = any, D = any>(url: string, data?: D, config?: any): Promise<T> {
    const response = await this.client.post<T>(url, data, config);
    return response.data;
  }

  async put<T = any, D = any>(url: string, data?: D, config?: any): Promise<T> {
...
export const httpClient: CommonHttpClient = new CommonHttpAxiosClientImpl(axiosClient);
